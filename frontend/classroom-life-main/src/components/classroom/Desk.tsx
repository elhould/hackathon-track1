...
interface DeskProps {
  ...
  chairRotation?: [number, number, number];
}

export function Desk({ position, deskRotation = [0, 0, 0], chairRotation = [0, 0, 0] }: DeskProps) {
  const deskColor = "#8b7355"; // Chunky wooden desk
  const legColor = "#6b5344";

  return (
    <group position={position}>
      {/* Desk - with its own rotation */}
      <group rotation={deskRotation}>
      ...
      {/* Chair - with its own rotation */}
      <group position={[0, 0, 0.9]} rotation={chairRotation}>
        {/* Seat */}
